// -----------------------------------------------------------------------------
// Виджет 'rule::Vertical'
// Графический виджет вертикальной разделительной линии.
// -----------------------------------------------------------------------------
...
inventory::submit! {
    AutoRegisteredWidget {
        name:        VRuleBlueprint::WIDGET_TYPE,
        category:    CAT_BASE,
        constructor: create_v_rule_creator,
    }
}

// Функция-помощник для создания Arc (вызовется автоматически внутри Factory::default)
fn create_v_rule_creator() -> Box<dyn WidgetCreator + Send + Sync> {
    log::info!("Авторегистрация '{}'", "VRule");
    Box::new(VRuleCreator)
}

// Конструктор blueprint для виджета
#[derive(Debug, Clone)]
pub struct VRuleCreator;

impl WidgetCreator for VRuleCreator {
    fn create_blueprint(&self, id: String) -> Rc<dyn WidgetBlueprint> {
        Rc::new(VRuleBlueprint::new(id))
    }
}
// -----------------------------------------------------------------------------
...
#[derive(Debug, Clone)]
pub struct VRuleBlueprint {
    pub meta:  CommonWidgetMeta,
    //pub props: RefCell<VRuleProps>,
}

impl HasCommonMeta for VRuleBlueprint {
...
impl VRuleBlueprint {
    ...
    const WIDGET_TYPE: &'static str = "v_rule";

    pub fn new(id: String) -> Self {
        Self {
            meta:  CommonWidgetMeta::new(id),
            //props: VRuleProps::default().into(),            
        }
    }

    pub fn parse_props(&self, factory: &Factory) -> VRuleProps {
        let widget_id = self.get_id();

        // Получить дефолтные значения
        let def = VRuleProps::default();
...
impl WidgetBlueprint for VRuleBlueprint {
    ...
    //crate::impl_refresh_props!(VRuleBlueprint, VRuleProps);
    ...
    fn build_element<'a>(
        ...
        // Создаем стандартный вертикальный разделитель Iced 0.14
        let base_rule = rule::vertical(props.thickness)
        ...
        let element: Element<'a, Message, Theme> = if factory.is_design_mode() {
            ...
            // В РЕЖИМЕ РАБОТЫ: Обычная пассивная вертикальная разделительная линия
            base_rule.into()
        };
    ...
    fn get_exportable_property_names(&self, factory: &Factory) -> Vec<PropertyKey> {
        ...
        let default = VRuleProps::default();
